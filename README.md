...
- One window tends the terrarium at a time: opening it elsewhere pauses the older window, and play (or painting) takes it back. A fullscreen button on the control row works from inside the site embed too.
- The terrarium autosaves and resumes on its own, growing a little while you are away — the last stretch of that growth plays out on screen, so you watch it wake up. First-time outcomes earn a one-line field note; each appears once ever. With the window open, tonight's date-seeded weather drifts in as real material — rain, settling snow, the occasional meteor — and the toggle under Room shuts it out. Clear, save/load a deliberate checkpoint, download/import a scene JSON file, export a postcard PNG, or export a short WebM clip from the right controls.
