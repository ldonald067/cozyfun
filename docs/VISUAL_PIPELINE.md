...
**A caveat that no floor can see: attunement is per-cell and never reaches a block's interior.** Each wellspring cell drinks a source touching IT, and an interior cell is surrounded by its own kind forever. Driving the real sim with a pour landing on the crown, and stable from 200 ticks to 3,000: **5/5 cells attune at radius 1, 12/13 at radius 2, 28/49 at the default brush, 60/197 at radius 8.** A painted block is an attuned shell around a permanently dormant heart, so the uniformly-attuned block the probe gates is an upper bound, exactly reachable only at radius 1-2. Whether that shell-and-core look is right is a design question nobody has answered on purpose.
