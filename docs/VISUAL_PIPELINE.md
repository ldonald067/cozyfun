...
- A plant's **silhouette** comes from its `variant`, exactly as its hue does: round bloom, daisy star, notched tulip, lavender spike, or bellflower. Heads are **five cells across**, which is the smallest head that can hold a shape at all — at three there are too few pixels to be anything but a block or a cross, and a 3-wide filled head reads as a coloured square. At five, knocked-off corners, a notched top edge and a checkered spike all read at play zoom.
