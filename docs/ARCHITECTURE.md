...
13. `app/src/buildWatch.ts` answers "is this tab still running the current build?" — the one question a long-open page cannot answer without asking. Pure comparison helpers plus a throttled check; see the measured argument in `docs/EMBEDDING.md`.
14. `app/src/assetUrl.ts` prefixes runtime-fetched asset URLs with the deploy base. It uses `import.meta`, so it MUST NOT be reachable from `engine.ts` or `materials.ts` — the parity harness compiles those two to CommonJS, where that is a compile error.
