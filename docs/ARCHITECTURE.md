...
11. `app/src/slowWorld.ts` converts an absence into slow steps. The rules themselves are engine-side (`Universe::slow_step`, mirrored in `engine.ts`); this module owns only the curve, so a harness can compile and measure it. Like `engine.ts` and `materials.ts` it MUST stay free of `import.meta` and of React.
12. `app/src/assetUrl.ts` prefixes runtime-fetched asset URLs with the deploy base. It uses `import.meta`, so it MUST NOT be reachable from `engine.ts` or `materials.ts` — the parity harness compiles those two to CommonJS, where that is a compile error.
