...
| Seed | Roots on soil when wet, or on another rooted seed, so water soaking down a bed roots the whole bed and it sprouts from its surface instead of stranding every buried grain; germinates into a climbing stalk that pushes up through open air or standing water and blooms a flower at its tip, taller with cosmic feeding; settles into the carpet when it lands wet on moss; rots into fungus under decay pressure; waits dormant when frozen; smothered by oil coating. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `water_soaks_down_through_a_seed_bed`, `a_seed_bed_sprouts_from_its_surface`, `a_capped_seed_never_sprouts`, `growth_pushes_up_through_standing_water`, `cosmic_fed_seed_grows_a_taller_stalk`, `frozen_seed_waits_instead_of_blooming`, `fungus_can_rot_wet_seed`, `oil_blocks_plain_water_hydration`; source: `update_seed` owns the moss-settle path. |
| Flower | Arrives as a single closed bud and unfurls petal by petal into a multi-cell head — the rooted crown at the middle, petals opened around it — in one of five garden hues chosen by the plant's variant; spends its energy puffing pollen motes from any open face on the head's rim, more often when cosmic, and dulls toward grey as that energy runs out; sheds spent petals one at a time as drifting motes, leaving the crown standing as a seed head. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `a_bud_opens_into_a_petal_crown`, `a_lone_petal_never_opens_a_head_of_its_own`, `a_capped_bloom_still_puffs_pollen_from_its_rim`, `a_spent_petal_sheds_as_a_drifting_mote`, `a_fresh_crown_holds_its_petals`, `mature_flower_releases_pollen`; visual QA: `material-identity-showcase` garden row; generated-only outcome. |
