...
| Seed | Roots on soil when wet with a soft sprout cue and a visibly growing shoot; germinates into a climbing stalk that blooms a flower at its tip, taller with cosmic feeding; rots into fungus under decay pressure; waits dormant when frozen; smothered by oil coating. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `frozen_seed_waits_instead_of_blooming`, `fungus_can_rot_wet_seed`, `oil_blocks_plain_water_hydration`. |
| Flower | Marks successful seeded growth; reacts visually to wet and cosmic states; spends energy releasing rare pollen motes once mature. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `mature_flower_releases_pollen`; visual QA: `material-identity-showcase`; generated-only outcome. |
...
| Spark | Flies outward from a rocket burst along its birth direction, then droops and falls as it fades; twinkles in firework hues and casts warm light; landing on rocket powder lights its fuse. | Tests: `rocket_burst_blooms_a_spark_shell_that_fades`; source: `update_spark` owns expansion, droop, and fade. |
