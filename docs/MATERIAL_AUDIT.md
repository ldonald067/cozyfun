...
| Flower | Arrives as a single closed bud and unfurls petal by petal into a multi-cell head, in one of five garden hues AND one of five five-cell-wide silhouettes — round bloom, daisy star, notched tulip, lavender spike, bellflower — both chosen by the plant's variant, and opening through its own drifting pollen so a big head is never left one petal short; spends its energy puffing pollen motes from any open face on the head's rim, more often when cosmic, and dulls toward grey as that energy runs out; sheds spent petals one at a time as drifting motes, leaving the crown standing as a seed head. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `a_bud_opens_into_a_petal_crown` (asserts two variants grow different silhouettes), `a_lone_petal_never_opens_a_head_of_its_own`, `a_capped_bloom_still_puffs_pollen_from_its_rim`, `a_spent_petal_sheds_as_a_drifting_mote`, `a_fresh_crown_holds_its_petals`, `mature_flower_releases_pollen`; visual QA: `material-identity-showcase` garden row; generated-only outcome. |
