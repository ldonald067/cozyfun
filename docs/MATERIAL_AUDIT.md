...
| Glass | Forms as a translucent pane where strong heat fuses dry sand; fogs with a field of micro-droplets streaked by clear runs, and beads steam back into water so a sealed glass terrarium keeps its moisture cycling; shatters back to sand with a chime under meteor impact. | Tests: `lava_vitrifies_dry_sand_into_glass`, `meteor_impact_vitrifies_nearby_sand`, `meteor_impact_shatters_glass_to_sand`, `glass_dew_fogs_the_pane_and_beads_into_water`; visual QA: `material-identity-showcase`. |
