...
| Soil | Falls as organic substrate — including off the edge of a one-cell pedestal, which is why every planter fixture in the tests needs a floor wide enough to hold it; stores moisture and greens into moss, except in ground a living seed is standing on — which neither greens on its own nor accepts moss spreading in from the side, and is released again once the seed dries out, so a watered bed no longer carpets over before anything can germinate; breathes a petrichor mist when watered after a long dry spell, venting from any open face so a pour from above does not sit on its own vent; roots wet seeds for blooming; feeds fungus decomposition; reborn where a starved fungus collapses, closing the soil to moss to fungus to soil loop. | Tests: `watered_soil_greens_up`, `soil_under_a_rooted_seed_stays_soil`, `rooted_seed_grows_a_stalk_that_blooms`, `first_water_on_dry_soil_breathes_mist`, `a_starved_old_fungus_collapses_into_soil`; source: `update_soil` and `update_fungus` own substrate behavior. |
...
| Seed | Roots on soil when wet, or on another rooted seed, so water soaking down a bed roots the whole bed and it sprouts from its surface instead of stranding every buried grain; germinates into a climbing stalk — but never in the shadow of a neighbouring plant, so a bed grows a spaced meadow instead of a solid wall — pushing up through open air or standing water and blooming at its tip, taller with cosmic feeding; settles into the carpet when it lands wet on moss; rots into fungus under decay pressure; waits dormant when frozen; smothered by oil coating. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `water_soaks_down_through_a_seed_bed`, `a_seed_bed_sprouts_from_its_surface`, `a_capped_seed_never_sprouts`, `growth_pushes_up_through_standing_water`, `cosmic_fed_seed_grows_a_taller_stalk`, `frozen_seed_waits_instead_of_blooming`, `fungus_can_rot_wet_seed`, `oil_blocks_plain_water_hydration`; source: `update_seed` owns the moss-settle path. |
| Flower | Arrives as a single closed bud and unfurls petal by petal into a multi-cell head, as one of eight species chosen by the plant's variant — cornflower, poppy, daisy, sunflower, tulip, lavender, bluebell, forget-me-not — each with its own silhouette, hue and centre (a poppy's heart is near-black, a sunflower's disc is brown), opening through its own drifting pollen so a big head is never left one petal short; spends its energy puffing pollen motes from any open face on the head's rim, more often when cosmic, and dulls toward grey as that energy runs out; sheds spent petals one at a time as drifting motes, leaving the crown standing as a seed head. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `a_bud_opens_into_a_petal_crown` (asserts two variants grow different silhouettes), `a_lone_petal_never_opens_a_head_of_its_own`, `a_capped_bloom_still_puffs_pollen_from_its_rim`, `a_spent_petal_sheds_as_a_drifting_mote`, `a_fresh_crown_holds_its_petals`, `mature_flower_releases_pollen`; visual QA: `material-identity-showcase` garden row; generated-only outcome. |
...
| Stem | Climbs upward cell by cell from a rooted seed, unfurling leaves on alternating sides as it rises, and blooms a flower at its tip; holds itself up on its own base or by clinging to a neighbouring stalk cell that has one, which is what keeps leaves attached while a severed stalk still collapses whole; burns and freezes like living growth. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `a_climbing_stalk_unfurls_side_leaves`, `a_leaf_rides_on_the_stalk_it_clings_to`, `cut_stalk_segments_fall`; visual QA: `material-identity-showcase`. |
| Glass | Forms as a translucent pane where strong heat fuses dry sand; fogs with a field of micro-droplets streaked by clear runs, and beads steam back into water so a sealed glass terrarium keeps its moisture cycling; shatters back to sand under meteor impact, the crack running one pane-width past the strike so a smashed window collapses instead of chipping. | Tests: `lava_vitrifies_dry_sand_into_glass`, `meteor_impact_vitrifies_nearby_sand`, `meteor_impact_shatters_glass_to_sand`, `glass_dew_fogs_the_pane_and_beads_into_water`; visual QA: `material-identity-showcase`. |
