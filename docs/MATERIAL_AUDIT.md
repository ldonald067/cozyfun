...
| Spark | Flies outward from a rocket burst along its birth direction, then droops and falls as it fades; twinkles in firework hues and casts warm light; reaching rocket powder during its outward flight lights its fuse. | Tests: `rocket_burst_blooms_a_spark_shell_that_fades`; source: `update_spark` owns expansion, droop, and fade. |
