...
| Seed | Roots on soil when wet with a soft sprout cue and a visibly growing shoot; blooms into flowers, faster with cosmic feeding; rots into fungus under decay pressure; waits dormant when frozen; smothered by oil coating. | Tests: `wet_seed_on_soil_blooms`, `frozen_seed_waits_instead_of_blooming`, `fungus_can_rot_wet_seed`, `oil_blocks_plain_water_hydration`. |
