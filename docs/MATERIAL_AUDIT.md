...
| Water | Flows and pools; hydrates soil, sand, and life; quenches lava and shocks meteor into scorched stone; simmers, bubbles, and boils away to steam over sustained flame, while hot water melts ice instead of freezing; rinses soot from scorched wall and stone; blocked from feeding life by oil coating. | Tests: `water_spreads_when_blocked`, `wet_seed_on_soil_blooms`, `water_quenches_lava_into_steam_and_stone`, `sustained_flame_simmers_then_boils_water`, `hot_water_melts_ice_and_resists_freezing`, `water_rinses_soot_from_hard_surfaces`. |
...
| Soil | Falls as organic substrate; stores moisture and greens into moss; breathes a petrichor mist when watered after a long dry spell; roots wet seeds for blooming; feeds fungus decomposition. | Tests: `watered_soil_greens_up`, `wet_seed_on_soil_blooms`, `first_water_on_dry_soil_breathes_mist`; source: `update_soil` and `update_fungus` own substrate behavior. |
...
| Moss | Spreads across damp soil and wood; colonizes damp stone slowly and soaked wall only when strongly fed; beads with visible dew when freshly watered and sheds droplets from overhangs; overtaken by fungus when old or wet; dries and scorches before burning. | Tests: `moss_colonizes_damp_stone`, `moss_needs_extra_energy_to_cross_wall`, `saturated_moss_drips_dew`, `heat_dries_wet_growth_before_burning`; visual QA: `material-identity-showcase` dewy moss. |
...
| Flower | Marks successful seeded growth; reacts visually to wet and cosmic states; spends energy releasing rare pollen motes once mature. | Tests: `wet_seed_on_soil_blooms`, `mature_flower_releases_pollen`; visual QA: `material-identity-showcase`; generated-only outcome. |
| Pollen | Drifts down as golden motes and settles where it lands; can take root as a seed on damp soil; fades quietly in the air. | Tests: `mature_flower_releases_pollen`, `pollen_seeds_wet_soil`; visual QA: `material-identity-showcase`. |
| Glass | Marks where strong heat fused dry sand; stays as an inert translucent pane; shatters back to sand with a chime under meteor impact. | Tests: `lava_vitrifies_dry_sand_into_glass`, `meteor_impact_vitrifies_nearby_sand`, `meteor_impact_shatters_glass_to_sand`; visual QA: `material-identity-showcase`. |
