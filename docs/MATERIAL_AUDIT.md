...
| Sand | Keep | Falling powder with wet clumping and drying behavior, and the one that shows density best: it settles through a pond into the lake bed that turns to sandstone. |
...
| Sand | [sand.pours] Pours fast as dry powder, two cells per tick; [sand.clumps] clumps and slows when wet; [sand.drains] drains dry back to loose grains; [sand.vitrifies] strong heat fuses dry grains into glass; [sand.settles] sinks through water and oil to the bed, lifting the liquid above it, so sand poured into a pond silts it into a lake bed. | Tests: `sand_falls`, `dry_sand_falls_two_cells_when_clear`, `wet_sand_still_falls_slowly`, `water_wets_sand_into_clumps`, `lava_vitrifies_dry_sand_into_glass`, `every_grain_sinks_through_a_pond_and_lifts_it`, `sand_sinks_through_an_oil_film_and_the_water_under_it`. |
...
| Flower | [flower.opens] Arrives as a single closed bud and unfurls petal by petal into a multi-cell head, as one of eight species chosen by the plant's variant — cornflower, poppy, daisy, sunflower, tulip, lavender, bluebell, cosmos — each with its own silhouette, hue and centre (a poppy's heart is near-black, a sunflower's disc is brown), opening through its own drifting pollen so a big head is never left one petal short; [flower.pollen] spends its energy puffing pollen motes from any open face on the head's rim, more often when cosmic, and dulls toward grey as that energy runs out; [flower.wilts] sheds spent petals one at a time as drifting motes, leaving the crown standing as a seed head. | Tests: `rooted_seed_grows_a_stalk_that_blooms`, `a_bud_opens_into_a_petal_crown` (asserts two variants grow different silhouettes), `a_lone_petal_never_opens_a_head_of_its_own`, `a_capped_bloom_still_puffs_pollen_from_its_rim`, `a_spent_petal_sheds_as_a_drifting_mote`, `a_fresh_crown_holds_its_petals`, `mature_flower_releases_pollen`; visual QA: `material-identity-showcase` garden row; generated-only outcome. |
...
**"Still water carves nothing" is too strong and the first draft of this section said it.**
The gate is *water with an empty neighbour*, which is a proxy for flow, not a proof of it —
and the surface of an OPEN pond has air above it, so it qualifies. That is why a stone bowl
still loses its waterline (169 → 146) before it holds. Only water with no empty neighbour at
