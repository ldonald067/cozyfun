...
## Step 1b — Preflight and the bar

`codex login status` must report a logged-in account. If it does not, stop and tell the user
to run `codex login` — do **not** silently fall back to same-model review, which is the one
thing this skill exists to prevent.

In a repository with a review bar of its own, read it and hand it to the reviewers: in
cozyfun that is `docs/CODE_REVIEW.md`, plus `docs/ARCHITECTURE.md` for the engine boundary
and `docs/VISUAL_PIPELINE.md` when the diff touches rendering. Reviewers get file contents,
not summaries.
...
State any CLAIM the author has made about the result ("steam no longer out-glows fire",
"parity holds"). An unproven claim is the single most valuable thing for a reviewer to
attack.
...
Then pick by CONTENT as well as size, where the repo has known risks. For cozyfun:

| Trigger | Add lens | Why |
| --- | --- | --- |
...
Cap at 4 reviewers. Read `references/reviewer-lenses.md` for all lens definitions, general
and repo-specific.
