...
| Asset | Local files | Source |
| --- | --- | --- |
| Favicon / touch icon | `app/public/favicon.svg`, `favicon.ico`, `apple-touch-icon.png` | Generated by `scripts/make-favicon.mjs` — no third party involved |

A cornflower drawn as a cell grid in that script and rendered to SVG/ICO/PNG with a
dependency-free encoder, following the pixel-flower reference the blooms themselves
were designed from. Its colours are **imported** from the game's own `SPECIES[0]` and
Stem palettes rather than copied, so a palette change moves the icon too — and
`npm run icons:check` fails the gate until it is regenerated with `npm run icons`.
Do not edit the binaries by hand.
