...
A cornflower drawn as a cell grid in that script, coloured from the game's own
`SPECIES[0]` and Stem palettes, and rendered to SVG/ICO/PNG with a dependency-free
encoder. It follows the pixel-flower reference the blooms themselves were designed
from. Regenerate with `node scripts/make-favicon.mjs` after any palette change rather
than editing the binaries.
